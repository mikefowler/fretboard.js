  ...
  synthControl.load(
    '#audio',
    {
      onEvent: ({ midiPitches, elements, ...rest }) => {
        ...
        const chordElement = elements[0].find(({ classList }) =>
          classList.contains('abcjs-annotation')
        );
        const noteElement = elements[0].find(({ classList }) =>
          classList.contains('abcjs-note')
        );
        ...
        if (!chordElement) {
          ...
          fretboard.style({
            filter: { inBox: true },
            text: ({ note, octave }) =>
              chord.indexOf(`${note}${octave}`) > -1 ? note : '',
            fill: ({ note, octave }) =>
              `${note}${octave}` === playedNote
                ? colors.chordTypes[chordType]
                : colors.defaultFill,
            stroke: ({ note, octave }) =>
              chord.indexOf(`${note}${octave}`) > -1
                ...
                : colors.defaultStroke,
            ['stroke-width']: ({ note, octave }) =>
              chord.indexOf(`${note}${octave}`) > -1 ? 2 : 1
          });
        }
